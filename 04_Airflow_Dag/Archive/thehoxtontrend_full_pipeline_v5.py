...
def save_scraped_data_to_postgres_db(**kwargs):
    ...
    # Read the CSV
    clean_description = pd.read_csv(io.StringIO(description_bytes), index=False, sep = ';')#, encoding='utf-8')
    
    log.info('passing description data from S3 bucket')
    
    clean_statistics = pd.read_csv(io.StringIO(statistics_bytes), index=False, sep = ';')
    
    log.info('passing statistics data from S3 bucket')
    
    clean_comments = pd.read_csv(io.StringIO(comments_bytes), index=False, sep = ';')
    
    log.info('passing comments data from S3 bucket')
    
    clean_tags = pd.read_csv(io.StringIO(tags_bytes), index=False, sep = ';')
    
    log.info('passing tags data from S3 bucket')
    
    clean_google_analytics = pd.read_csv(io.StringIO(google_analytics_bytes), index=False, sep = ',')
...
create_schema >> youtube_web_scraping_task >> save_scraped_data_to_s3_task >> save_scraped_data_to_postgres_db_task
