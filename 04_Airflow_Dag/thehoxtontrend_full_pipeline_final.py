...
default_args = {
    ...
    'description_table_key': Variable.get("thehoxtontrend", deserialize_json=True)['description_table_key'] + time.strftime("%Y_%m_%d") + '.csv',
    'statistics_table_key': Variable.get("thehoxtontrend", deserialize_json=True)['statistics_table_key'] + time.strftime("%Y_%m_%d") + '.csv',
    'comments_table_key' : Variable.get("thehoxtontrend", deserialize_json=True)['comments_table_key'] + time.strftime("%Y_%m_%d") + '.csv',
    'tags_table_key' : Variable.get("thehoxtontrend", deserialize_json=True)['tags_table_key'] + time.strftime("%Y_%m_%d") + '.csv',
...
def s3_save_file_func(**kwargs):
    ...
    scraped_data_previous_task = task_instance.xcom_pull(task_ids="youtube_web_scraping")
...
def save_scraped_data_to_postgres_db(**kwargs):
    ...
    comments_table_key = kwargs['comments_table_key']
    tags_table_key = kwargs['tags_table_key']
    # google_analytics_table_key = kwargs['google_analytics_table_key']
    s3 = S3Hook(kwargs['aws_conn_id'])
    ...
    # Read the content of the key from the bucket
    description_bytes = s3.read_key(description_table_key, bucket_name)
    statistics_bytes = s3.read_key(statistics_table_key, bucket_name)
    comments_bytes = s3.read_key(comments_table_key, bucket_name)
    tags_bytes = s3.read_key(tags_table_key, bucket_name)
    # google_analytics_bytes = s3.read_key(google_analytics_table_key, bucket_name)
    
    # Read the CSV
    clean_description = pd.read_csv(io.StringIO(description_bytes), sep = ';')#, encoding='utf-8')
    
    log.info('passing description data from S3 bucket')
    
    clean_statistics = pd.read_csv(io.StringIO(statistics_bytes), sep = ';')
    
    log.info('passing statistics data from S3 bucket')
    
    clean_comments = pd.read_csv(io.StringIO(comments_bytes), sep = ';')
    
    log.info('passing comments data from S3 bucket')
    
    clean_tags = pd.read_csv(io.StringIO(tags_bytes), sep = ';')
    
    log.info('passing tags data from S3 bucket')
    
    # clean_google_analytics = pd.read_csv(io.StringIO(google_analytics_bytes), sep = ',')
    
    # log.info('passing google analytics data from S3 bucket')
    ...
    s1 = """INSERT INTO analytics_schema.video_descriptions(date, video_id, video_title, video_description) VALUES (%s, %s, %s, %s)"""
    for index in range(len(clean_description)):
        obj1 = []

        obj1.append([clean_description.date[index],
                   clean_description.video_id[index],
                   clean_description.video_title[index],
                   clean_description.video_description[index]])

        cursor.executemany(s1, obj1)
        conn.commit()

    log.info('Finished saving the description data to postgres database')

    s2 = """INSERT INTO analytics_schema.video_statistics(video_id, views, likes, dislikes, comments) VALUES (%s, %s, %s, %s, %s)"""
    for index in range(len(clean_statistics)):
        obj2 = []

        obj2.append([clean_statistics.video_id[index],
                   clean_statistics.views[index],
                   clean_statistics.likes[index],
                   clean_statistics.dislikes[index],
                   clean_statistics.comments[index]])

        cursor.executemany(s2, obj2)
        conn.commit()

    log.info('Finished saving the statistics data to postgres database')

    s3 = """INSERT INTO analytics_schema.video_comments(date, video_id, comment, author, likes, reply_flag) VALUES (%s, %s, %s, %s, %s, %s)"""
    for index in range(len(clean_comments)):
        obj3 = []

        obj3.append([clean_comments.date[index],
                   clean_comments.video_id[index],
                   clean_comments.comment[index],
                   clean_comments.author[index],
                   clean_comments.likes[index],
                   clean_comments.reply_flag[index]])

        cursor.executemany(s3, obj3)
        conn.commit()

    log.info('Finished saving the comments data to postgres database')    

    s4 = """INSERT INTO analytics_schema.video_tags(video_id, tag_number, tag_value) VALUES (%s, %s, %s)"""
    for index in range(len(clean_tags)):
        obj4 = []

        obj4.append([clean_tags.video_id[index],
                   clean_tags.tag_number[index],
                   clean_tags.tag_value[index]])

        cursor.executemany(s4, obj4)
        conn.commit()

    log.info('Finished saving the tags data to postgres database')    

    # s5 = """INSERT INTO analytics_schema.website_statistics(date, average_session_duration, bounce_rate, new_users, number_of_sessions_per_user, pages_per_session, page_views, users) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"""
    # for index in range(len(clean_google_analytics)):
    #     obj5 = []

    #     obj5.append([clean_google_analytics.date[index],
    #                clean_google_analytics.average_session_duration[index],
    #                clean_google_analytics.bounce_rate[index],
    #                clean_google_analytics.new_users[index],
    #                clean_google_analytics.number_of_sessions_per_user[index],
    #                clean_google_analytics.pages_per_session[index],
    #                clean_google_analytics.page_views[index],
    #                clean_google_analytics.users[index]])

    #     cursor.executemany(s5, obj5)
    #     conn.commit()

    # log.info('Finished saving the website analytics data to postgres database')    

# Function to create a Regression Model with AWS Lambda to predict the number of comments a video will accumulate
# and make Regression Model's results accessible via AWS API Gateway 
def lambda_create_and_share_regression_model(**kwargs):
  hook = AwsLambdaHook('create_and_share_regression_model',
                      region_name='eu-west-2',
                      log_type='None', qualifier='$LATEST',
                      invocation_type='RequestResponse',
                      config=None, aws_conn_id='aws_default_chigozienlewedum')
...
save_scraped_data_to_postgres_db_task = PythonOperator(
    task_id='save_scraped_data_to_postgres_db',
    provide_context=True,
    python_callable=save_scraped_data_to_postgres_db,
    trigger_rule=TriggerRule.ALL_SUCCESS,
    op_kwargs=default_args,
    dag=dag,
...
lambda_create_and_share_regression_model_task = PythonOperator(
    task_id='lambda_create_and_share_regression_model',
    provide_context=True,
    python_callable=lambda_create_and_share_regression_model,
    trigger_rule=TriggerRule.ALL_SUCCESS,
    op_kwargs=default_args,
    dag=dag,
...
create_schema_task >> youtube_web_scraping_task >> save_scraped_data_to_s3_task >> save_scraped_data_to_postgres_db_task >> lambda_create_and_share_regression_model_task
